# DISCLAIMER: This code is adapted from the sktime library.
# Original source: https://github.com/sktime/sktime/blob/main/sktime/datasets/_readers_writers/ts.py
# Modifications: Added lines 283 - 290 to enable continuous regression targets.
# Original sktime code is licensed under BSD 3-Clause License.
                                ...
                                if line[char_num] == ":":
                                    ...
                                    if line[char_num] != "(" and class_labels:
                                        ...
                                        if "class_label_list" in locals():
                                            ...
                                            # Regression: validate that it can be cast to y_dtype
                                            dtype_map = {"str": str, "int": int, "float": float}
                                            if y_dtype not in dtype_map:
                                                raise ValueError(f"Invalid y_dtype: {y_dtype}. Must be 'str', 'int', or 'float'")
                                            try:
                                                dtype_map[y_dtype](class_val)
                                            except (ValueError, TypeError) as e:
                                                raise OSError(f"the target value '{class_val}' on line {line_num + 1} cannot be converted to dtype: {y_dtype}. Error: {e}")
